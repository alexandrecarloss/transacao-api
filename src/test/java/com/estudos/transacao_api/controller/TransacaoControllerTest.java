...
public class TransacaoControllerTest {
    ...
    void setup(){
        ...
        transacao = new TransacaoRequestDTO(20.0, OffsetDateTime.of(2026, 1, 24, 18, 50, 0, 0, ZoneOffset.UTC)) ;
    }

    @Test
    void DeveAdicionarTransacaoComSucesso() throws Exception {
        doNothing().when(transacaoService).adicionarTrasacoes(transacao);
    ...
    void DeveGerarExcecaoAoAdicionarTransacao() throws Exception {
        doThrow(new UnprocessableEntity("Erro de requisição")).when(transacaoService).adicionarTrasacoes(transacao);
        mockMvc.perform(post("/transacao")
                        .content(objectMapper.writeValueAsString(transacao))
                        .contentType(MediaType.APPLICATION_JSON))
